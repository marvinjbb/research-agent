...
class EvidenceAggregator:
    ...
    def aggregate(self, execution: ResearchExecutionResult) -> EvidenceBundle:
        successful = [
            ...
            if outcome.status is WorkerExecutionStatus.SUCCEEDED and outcome.result is not None
        ]
        if not successful:
        ...
        for outcome in successful:
            ...
            for claim in result.claims:
                for evidence in claim.evidence:
                    excerpts_by_source.setdefault(evidence.source_id, []).append(evidence.evidence)
            ...
            for source in result.sources:
                ...
                source_id_map[(result.worker_id, source.source_id)] = source_data["source_id"]

        sources = [AggregatedSource.model_validate(source) for source in sources_by_url.values()]
            ...
            for claim in result.claims:
                ...
                for evidence in claim.evidence:
                    aggregated = AggregatedEvidence(
                        ...
                        source_id=source_id_map[(result.worker_id, evidence.source_id)],
                        evidence=evidence.evidence,
                    )
                    if aggregated not in claim_data["evidence"]:
                        claim_data["evidence"].append(aggregated)

        claims = [AggregatedClaim.model_validate(claim) for claim in claims_by_text.values()]
        overlaps = self._find_obvious_overlaps(claims)
        uncertainties = [
            AggregatedUncertainty(
                uncertainty_id=(
                    "uncertainty-"
                    + sha256(f"{outcome.worker_id}\0{uncertainty}".encode()).hexdigest()[:28]
                ),
        ...
        failed_workers = [
            ...
            for outcome in execution.workers
            if outcome.status is WorkerExecutionStatus.FAILED and outcome.error is not None
        ]
        ...
        for index, left in enumerate(claims):
            ...
            for right in claims[index + 1 :]:
                ...
                if left_text in right_text or right_text in left_text:
                    overlaps.append(ClaimOverlap(claim_ids=[left.claim_id, right.claim_id]))
        return overlaps
